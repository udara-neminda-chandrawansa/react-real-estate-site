...
function CardType2({ cardText1, cardText2, cardUrl, cardImage, cardImageMd, extraClasses }) {
    ...
    return (
        <div className={`flex flex-col items-center justify-between ${extraClasses}`}>
            ...
            <span className="flex w-full py-3 gap-3 g-font-2 uppercase text-xs tracking-widest">
                <a href={cardUrl} className="w-3/4 max-md:w-fit max-sm:w-1/3">{cardText1}</a>
                <p className="w-1/4 px-2 border-l-[1px] border-l-black max-md:w-fit max-sm:w-2/3 max-sm:h-fit">{cardText2}</p>
