function Button({ displayText, url, theme, width, onClick, noPadding, extraClasses, extraClasses4Button }) {
    ...
    const buttonTextColor = theme === 'dark' ? 'text-white' 
                          : theme === 'transparent' ? 'text-black'
                          : 'text-black';

    const buttonBorderColor = theme === 'dark' ? 'border-black hover:border-white' 
                           : theme === 'transparent' ? 'border-none'
